...
class THWCFE_License_Manager {
	...
	public function __construct($file, $api_url, $software_type='plugin', $software_title=false, $software_version=false, $text_domain=''){
		if(is_admin()){
			...
			// Use thlm_site_transient_update_sw_hook_priority filter in custom plugin
			$site_transient_update_sw_hp = apply_filters('thlm_site_transient_update_sw_hook_priority', 999, $this->identifier, $this->sw_prefix);
			...
			if($this->software_type == 'plugin'){
				...
				add_filter('pre_set_site_transient_update_plugins', array($this, 'check_software_updates'), $site_transient_update_sw_hp);
				...
				// Theme update
				add_filter('pre_set_site_transient_update_themes', array($this, 'check_software_updates'), $site_transient_update_sw_hp);
