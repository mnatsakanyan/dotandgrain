<!-- <script src="<?php echo cc_wc_script_url('bootstrap/bootstrap.min.js') ?>"></script> -->
<!-- <link rel="stylesheet" href="<?php echo cc_wc_assets_url('bootstrap/bootstrap.min.css') ?>"> -->
<!-- <link rel="stylesheet" href="<?php echo cc_wc_assets_url('bootstrap/bootstrap.fix.css') ?>"> -->

<div id='personalize_options' class='panel woocommerce_options_panel'>
    <?php /** @var TYPE_NAME $model */
    if ($model->IsNew) : ?>
        <div class="bootstrap-panel panel-default">
            <div class="panel-body">
                <p><?php echo __('Please, save the product before setting up personalization.', 'customers-canvas-for-wc') ?></p>
            </div>
        </div>
    <?php else : ?>
        <div class='options_group'>
            <script>
                var CcProductModel = <?php echo $model->JsonModel ?>;
            </script>

            <style>
                au-attribute-manager {
                    width: 800px;
                }

                div#editor.panel-body {
                    display: flex;
                    justify-content: center;
                }

                button.attribute-manager-reset.mat-button {
                    display: none;
                }
            </style>

            <script>
            function toggleInstructions() {
                let elem = document.querySelector("a#instructionButton");
                elem.state = !elem.state;
                document.querySelector("#collapseOne").style.display = elem.state ? "block" : "none";
            }

            </script>
            
            <script src="<?php echo cc_wc_assets_url('js/attributeMigrator.js') ?>"></script>

            <div class="bootstrap-panel panel-default" style="padding-left: 50px;">
                <div class="panel-heading" role="tab" id="headingOne">
                    <h4 class="panel-title">
                        <a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapseOne"
                            aria-expanded="true" aria-controls="collapseOne" id="instructionButton" onclick="toggleInstructions()">
                            <?php echo __('Click for instructions', 'customers-canvas-for-wc') ?>
                        </a>
                    </h4>
                </div>
                <div id="collapseOne" style="display: none" class="panel-collapse collapse" role="tabpanel" aria-labelledby="headingOne">
                    <div class="panel-body">
                        <p><?php echo __('To associate this product with Aurigma Personalization Plugin:', 'customers-canvas-for-wc') ?></p>
                        <ol>
                            <li><?php echo __('Choose an Editor from a list', 'customers-canvas-for-wc') ?></li>
                            <li><?php echo __('Select its config (a set of predefined parameters)', 'customers-canvas-for-wc') ?></li>
                            <li><?php echo __('Specify variable attributes if necessary', 'customers-canvas-for-wc') ?></li>
                        </ol>
                        <p><?php echo __('Then click Apply to generate all necessary Specification Attributes', 'customers-canvas-for-wc') ?></p>
                        <p><?php echo __('To unlink the product from the Aurigma Personalization Plugin, click Unlink button - note, it will delete Aurigma\'s Specification Attributes from this product.', 'customers-canvas-for-wc') ?></p>
                        <hr/>
                    </div>
                </div>
            </div>
            <script>

                document.addEventListener("DOMContentLoaded", function() {
                    let iframe = document.createElement("iframe");
                    let editorDiv = document.querySelector("div#editor");
                    iframe.id="attribute-manager-iframe";
                    iframe.onload=attributeManagerIframeLoaded;
                    iframe.width="100%";
                    iframe.height="100%";
                    iframe.src = "about:blank";
                    iframe.style="border: none; height: 100%; width: 100%;";
                    editorDiv.appendChild(iframe);
                });
                
                function getIframeDoc() {
                    let iframe = document.querySelector('#attribute-manager-iframe');
                    let iframeWin = iframe.contentWindow || iframe;
                    let iframeDoc = iframe.contentDocument || iframeWin.document;
                    return iframeDoc;
                }
                
                function attributeManagerIframeLoaded() {                   
                    let iframeDoc = getIframeDoc();

                    let urls = [
                        "<?php echo cc_wc_assets_url('attribute-manager/au-attribute-manager.js') ?>",
                        "<?php echo cc_wc_assets_url('attribute-manager/styles.css') ?>",
                        "https://fonts.googleapis.com/icon?family=Material+Icons"
                    ]

                    let arr = [
                        "<script src=" + urls[0] + "><\/script>",
                        '<link rel="stylesheet" href="' + urls[1] + '">',
                        // '<link rel="stylesheet" href="' + urls[2] + '">',
                        // hide 'reset all' button
                        '<style>au-attribute-manager {width: 800px;}div#editor.panel-body {display: flex;justify-content: center;}button.attribute-manager-reset.mat-button {display: none;}</style>'
                    ];

                    for(let i=0; i < arr.length; i++) {
                        iframeDoc.write(arr[i]);
                    }

                    iframeDoc.write('\<au-attribute-manager id="manager">\<\/au-attribute-manager>');

                    getAttributeManagerElement().then(initManager);
                }

                function getAttributeManagerElement() {
                    return new Promise(function(resolve, reject) {
                        let timeoutFunc = function() {
                            setTimeout(function()  {
                                let iframeDoc = getIframeDoc();
                                const manager = iframeDoc.querySelector("au-attribute-manager");
                                if (manager) {
                                    resolve(manager);
                                } else {
                                    timeoutFunc();
                                }
                            }, 200);
                        };
                        timeoutFunc();
                    });
                }

                function fixIframeLoad() {
                    let iframe = document.querySelector('#attribute-manager-iframe');
                    if (iframe.contentWindow.stop) {
                        iframe.contentWindow.stop() 
                    }
                    else { 
                        if (iframe.contentWindow.document.execCommand) {
                            iframe.contentWindow.document.execCommand("Stop") 
                        }
                        else {
                            iframe.src = "about:blank";
                        }
                    }
                }

                function setEditorParentHeight() {
                    let parentDiv = document.querySelector("div#editor");
                        parentDiv.style.height = "800px";
                }

                function initManager(attributeManager) {
                    const editorAttrName = "editor";
                    const configAttrname = "editor_config";
                    let selectedConfigName = "";
                    let selectedEditorName = "";

                    attributeManager.title = "";
                    attributeManager.subTitle = "";
                    const migrator = new window.attributeMigrator.Migrator(CcProductModel.CcUrl, CcProductModel.id, "woocommerce");
                    migrator.migrateEditors(CcProductModel.EditorList).then(function (editors) {
                        document.querySelector("#editor img").remove();

                        fixIframeLoad();

                        const editorMetafield = migrator.metafields.find(x => x.name === editorAttrName);
                        const configMetafield = migrator.metafields.find(x => x.name === configAttrname);

                        if (editorMetafield) {
                            let editor = editors.find(function (x) {
                                return x.title === editorMetafield.value
                            });
                            if (editor) {
                                selectedEditorName = editor.name;
                                setEditorParentHeight();
                            }
                        }
                        selectedConfigName = configMetafield ? configMetafield.value : "";

                        attributeManager.configuration = {
                            selectedEditorName: selectedEditorName,
                            selectedConfigName: selectedConfigName,
                            editors: editors,
                            handleOutOfSchemeValues: false,
                            editorChanged: false
                        };

                        attributeManager.values = migrator.metafields
                            .map(function (metafield) {
                                return {name: metafield.name, value: metafield.value};
                            })
                            .filter(function (x) {
                                return [editorAttrName, configAttrname].indexOf(x.name) === -1;
                            });
                    });

                    attributeManager.addEventListener('editorChange', function (event) {
                        let editor = event.detail;
                        selectedEditorName = editor.title;
                    });

                    attributeManager.addEventListener('editorConfigChange', function (event) {
                        let config = event.detail;
                        selectedConfigName = config.name;

                        setEditorParentHeight();
                    });

                    let prepareAttributesForServer = function (event) {
                        const attributes = event.detail.data;
                        let newAttributes = attributes.map(attr => {
                            return {
                                productId: CcProductModel.id,
                                name: attr.name,
                                value: attr.value
                            }
                        });
                        var selectedEditor = attributeManager.configuration.editors.filter(x => x.name == selectedEditorName)[0];
                        if (selectedEditor && !attributeManager.configuration.editorChanged) {
                        	selectedEditorName = selectedEditor.title;
                            attributeManager.configuration.editorChanged = true;
                        }
                        newAttributes.push({
                            productId: CcProductModel.id,
                            name: editorAttrName,
                            value: selectedEditorName
                        });
                        newAttributes.push({
                            productId: CcProductModel.id,
                            name: configAttrname,
                            value: selectedConfigName
                        });
                        return newAttributes;
                    };

                    // listen submit
                    attributeManager.addEventListener('submit', event => {
                        let attributes = prepareAttributesForServer(event);
                        ajPost("/wp-admin/admin-ajax.php?action=save_cc_attribute", {"attributes": attributes})
                            .then(function (value) {
                                event.detail.complete();
                            });
                    });

                    // listen cancel
                    attributeManager.addEventListener('cancel', event => {
                        let attributes = prepareAttributesForServer(event);
                        ajPost("/wp-admin/admin-ajax.php?action=delete_spec_attribute", {"list": attributes})
                            .then(function (value) {
                                event.detail.complete();
                            });
                    });

                    async function ajPost(url, body) {
                        url = window.location.href.split("/wp-admin/")[0] + url;
                        let res = await fetch(url, {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json'
                            },
                            body: JSON.stringify(body)
                        });
                        return await res.json();
                    }

                    let img = document.createElement("img");
                    img.style.width = "200px";
                    img.style.height = "150px";
                    img.src = "data:image/gif;base64,";
                    document.querySelector("#editor").appendChild(img);
                };
            </script>
            <div class="bootstrap-panel panel-default">
                <div id="editor" class="panel-body" 
                    style="text-align: center; padding-left: 50px; overflow: hidden; height:420px; position: relative;">
                <!-- <iframe id="attribute-manager-iframe" 
                    onload="attributeManagerIframeLoaded()" 
                    width="100%" height="100%" 
                    style="border: none; height: 100%;left: 0;position: absolute;top: 0;width: 100%;"></iframe> -->
            </div>
            </div>
        </div>
    <?php endif; ?>
</div>
