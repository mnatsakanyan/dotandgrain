...
//*** checking and assigning data
    ...
    public function admin_enqueue_scripts() {
        if (isset($_GET['tab']) AND $_GET['tab'] == 'woocs' AND!isset($_GET['design'])) {
...
//+++
        ...
        if (isset($_GET['design']) AND $_GET['design'] === 'new') {

            wp_enqueue_style('woocs2-bootstrap', WOOCS_LINK . 'new-admin/bootstrap.min.css', array(), WOOCS_VERSION);
            wp_enqueue_style('woocs2-unicons', 'https://unicons.iconscout.com/release/v3.0.6/css/line.css', array(), WOOCS_VERSION);
            wp_enqueue_style('woocs2-general', WOOCS_LINK . 'new-admin/general.css', array('woocs2-bootstrap'), WOOCS_VERSION);

            echo $this->render_html(WOOCS_PATH . 'views/plugin_options_new.php', $args);
        } else {
            ...
            wp_localize_script('woocommerce-currency-switcher-options', 'woocs_lang', [
                'blind_option' => __("Native WooCommerce price filter does not see data generated by this feature.", 'woocommerce-currency-switcher'),
                'loading' => __("Loading", 'woocommerce-currency-switcher'),
                'save_changes' => __("Save changes please!", 'woocommerce-currency-switcher')
            ]);
    ...
    public function ask_favour() {
        ...
        add_action('admin_notices', function() use($slug) {
            ...
            <div class="notice notice-info woocs-pos-relative" id="pn_<?php echo $slug ?>_ask_favour">
                <button onclick="javascript: pn_<?php echo $slug ?>_dismiss_review(1); void(0);" title="<?php _e('Later', 'woocommerce-currency-switcher'); ?>" class="notice-dismiss"></button>
                <div id="pn_<?php echo $slug ?>_review_suggestion">
                    ...
                    <p><a href="javascript: pn_<?php echo $slug ?>_set_review(1); void(0);"><?php _e('Yes, I love it', 'woocommerce-currency-switcher'); ?></a> 🙂 | <a href="javascript: pn_<?php echo $slug ?>_set_review(0); void(0);"><?php _e('Not really...', 'woocommerce-currency-switcher'); ?></a></p>
                </div>
                ...
                <div id="pn_<?php echo $slug ?>_review_no" style="display: none;">
                    <p><?php _e('We are sorry to hear you aren\'t enjoying WOOCS. We would love a chance to improve it. Could you take a minute and let us know what we can do better?', 'woocommerce-currency-switcher'); ?></p>
                    <p>
